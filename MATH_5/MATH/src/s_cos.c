...
/*
	s_cos.c是对余弦函数在不同输入范围下的计算方法的一种高效实现，适用于所有实数输入。
...
{
	/*
		y[0]:存储将输入规约到[-π/4, π/4]的计算结果，y[1]为了保持精度额外计算的高精度尾数部分。
		n:规约过程中加减π/2的个数
	*/
	double y[2],z=0.0;
	int32_t n, ix;
	...
	如果x很小（小于2^{-27}，即0x3e46a09e），
	如果x为0直接返回1.0（为了处理很小的值时的精度问题）。
	...
	函数返回 NaN。这是因为余弦函数对于无穷大和NaN没有定义。*/
	...
	具体映射如下：
     当n = 0：使用__kernel_cos直接计算余弦值。
     当n = 1：使用__kernel_sin计算正弦值并取负，作为余弦值（因为在第二象限，余弦值等于负的正弦值）。
     当n = 2：使用__kernel_cos计算余弦值并取负（因为在第三象限，余弦值为负）。
 	 当n != 0\1\2时：使用__kernel_sin计算正弦值，作为余弦值（因为在第四象限，余弦值等于正弦值）。*/
	else {
